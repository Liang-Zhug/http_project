...
private: // 接收请求内部使用的接口
  ...
  bool IsNeedToRecvRequestBody() {
    ...
    if (_httprequest._method == "POST") {
      ...
      _httprequest._contentLenth =
          std::stoi(_httprequest._headkv["Content-Length"]);
      if (_httprequest._contentLenth != 0) {
        LOG(INFO, "POST method, recv Content-Length: " +
                      _httprequest._headkv["Content-Length"]);
        return true;
      }
...
public:
  bool RecvRequest() // 接受请求并分析请求
  {
    bool end = false;
    RecvRequestLine(); // 接受请求行
    if (stop == false) {
      // 请求行接收未出错就继续
      RecvRequestHeader(); // 接收请求报头
      if (stop == false) {
        parseRequestLine(); // 解析请求行，找出请求方法 uri 协议版本
        PraseRequsetHeader(); // 将请求报头中的kv全部放入哈希
...
private: // 响应内部接口
  // 分析是GET还是POST，是POST参数就全在正文body里，是GET则在uri里
    size_t ProcessCgi() {
    /*cgi整体流程：创建子进程，让cgi进程替换掉子进程，父进程request中的参数传给cgi进程
    cgi进程处理完后将结果返回给父进程*/
    auto &path = _httprequest._path;    
    // 要想让父子进程通信，直接用管道来实现
    ...
    {
      // 创建input管道失败
      LOG(ERROR, "create input pipe failed, strerror: " +
                     std::string(strerror(errno)));
      return 500;
    }
    if (pipe(output) < 0) // 创建output管道
    {                     // 创建output管道失败
      LOG(ERROR, "create output pipe failed, strerror: " +
                     std::string(strerror(errno)));
      return 500;
    }
    ...
    if (pid == 0) { // 子进程
      // 关掉不需要的文件描述符
      //父进程用input[0]读，用output[1]写
      //子进程用input[1]读，用output[0]写
      close(input[0]);
      close(output[1]);

      // ==============================================
      // 带参GET用环境变量传递参数
      if (_httprequest._method == "GET") { // 方法是带参GET
        // 将queryString转到环境变量中
        std::string queryString_env =
            "QUERY_STRING=" + _httprequest._queryString;
        ...
        if (putenv((char *)queryString_env.c_str()) != 0) {
          ...
          _httprequest._method ==
          "POST") { // POST方法，要指明传入了多少字节的参数，数字不会很大，也用环境变量来给
        std::string contentLength_env =
            "CONTENT_LENGTH=" + std::to_string(_httprequest._contentLenth);
      ...
      dup2(
          output[0],
          0); // 替换之后的进程，文件描述符0就表示原先的output[0]，也就是子进程专门读取的管道的文件描述符
      dup2(
          input[1],
          1); // 替换之后的进程，文件描述符1就表示原先的input[1]，也就是子进程专门读取的管道的文件描述符
    ...
    */
      ...
      exit(
          1); // 如果替换异常就会执行这句，此时子进程的退出码就是1，根据这个来让父进程判断子进程是否成功替换
      ...
      // ============================================== POST用管道传递参数
      if (_httprequest._method ==
          "POST") { // 方法是POST，将body中的内容全部写到output[1]中
        size_t total = 0, size_singleTime = 0;
        size_t size = _httprequest._requestBody.size();
        while (total < size &&
               (size_singleTime =
                    write(output[1], _httprequest._requestBody.c_str(), size) >
                    0)) {
          total += size_singleTime;
        }

        if (total != size) {
          LOG(ERROR, "write size[" + std::to_string(total) + "] != body size[" +
                         std::to_string(size) + ']');
          return 500;
        }
      }
      ...
      if (pid == _pid) {
        if (WIFEXITED(status)) { // 正常退出，即调用exit、_exit、return退出的
          if (WEXITSTATUS(status) !=
              0) { // 退出码不为0，上面替换失败了，或者是替换后的进程退出码不为零，返回
        ...
        } else { // 子进程没有正常退出，可能是信号终止了啥的
          return 400;
        }
      } else { // 就一个子进程，如果等待到的进程id和前面子进程的id不同，那就出错了
        return 500;
      }
